...
function comment(selection, document, activeEditor) {
  return new Promise(async resolve => {
    ...
    // 获取选择文本中所有`<!--`和`-->`的位置
    const commentPositions = getCommentPositions(word, start, end, "<!--", "-->")

    // 获取选择文本中所有`/*`和`*/`的位置
    const styleCommentPositions = getCommentPositions(word, start, end, "/*", "*/")
...
/** 选中内容没有注释，直接调用注释命令 */
async function noCommentPositionsDeal(activeEditor, start, end, originStart, originEnd) {
