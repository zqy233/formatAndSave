...
const extensionPrettier = hx.commands.registerCommand('extension.prettier', async () => {
  ...
  // 如果当前文件就是配置文件需要先保存(路径首字母不一样，第一个是d:,第二个是D:)
  if (fileName.toLowerCase() === configFilePath.toLowerCase()) {
    await hx.commands.executeCommand('workbench.action.files.save')
  }
  // 未找到返回null，则创建配置文件
  if (!configFilePath) {
    try {
      const defaultConfig = await fs.readFile(path.resolve(hx.env.appRoot, 'plugins', 'zqy-formatAndSave',
        '.prettierrc.js'), {
        encoding: 'utf-8'
      })
      try {
        await fs.writeFile(path.resolve(workspaceFolder.uri.fsPath, '.prettierrc.js'), defaultConfig)
        hx.window.showInformationMessage('formatAndSave创建prettier配置文件成功，文件路径： ' + path.resolve(workspaceFolder.uri
          .fsPath, '.prettierrc.js'))
      } catch (err) {
        return hx.window.showErrorMessage("formatAndSave创建默认配置文件出错 " + err)
      }
      configFilePath = path.resolve(__dirname, '.prettierrc.js')
    } catch (error) {
      return hx.window.showErrorMessage("formatAndSave读取插件默认配置文件出错 " + error)
    }
  }
  // 解析配置文件信息为js对象
