...
// 跳转至上一个""处
const jumpLastQuote = hx.commands.registerCommand("extension.jumpLastQuote", async () => {
...
// 跳转至下一个""处
const jumpNextQuote = hx.commands.registerCommand("extension.jumpNextQuote", async () => {
  let activeEditor = await hx.window.getActiveTextEditor() // 获取当前编辑器
  const word = activeEditor.document.getText() // 当前编辑器的所有文本
  const positions = getAllIndex(word, "\"")
  const cursorPosition = activeEditor.selection.active
